    ...
    <div className="p-[20px] border-[2px] rounded-xl">
      <div onClick={handleClick} className="cursor-pointer flex flex-col gap-4">
        ...
        <div className="grid grid-cols-7 gap-4">
          <div className="col-span-5 flex flex-col gap-2">
            ...
            <p className="text-[14px] text-slate-400 line-clamp-2">
              {item?.previewContent && parse(item?.previewContent)}
            </p>
            <div>
              <span className="text-[12px] font-medium">
                {convertTimeToAgo(item?.createdAt)}
              </span>
            </div>
          </div>
          <div className="col-span-2">
            <div className="h-[180px] w-full rounded-2xl overflow-hidden">
              <img
                className="w-full h-full object-cover"
                src="https://images.pexels.com/photos/19762800/pexels-photo-19762800/free-photo-of-qu-n-short-ng-i-cai-gh-tieu-di-m.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                alt=""
              />
            </div>
          </div>
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          className="z-10 flex items-center gap-3 bg-green-500 text-white w-fit p-[8px] rounded-lg"
        >
          <FaCheck />
        ...
        <button
          ...
          className="z-10 flex items-center gap-3 bg-red-500 text-white w-fit p-[8px] rounded-lg"
        >
          <IoMdCloseCircleOutline />
          <div className="text-[14px]">Không được kiểm duyệt</div>
        </button>
      </div>
      {showReason && <CheckBlog id={item?._id} setShowReason={setShowReason} />}
    </div>
  );
};
