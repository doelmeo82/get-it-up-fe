    ...
    <div className="pt-[100px] pb-[60px] px-[24px] max-w-[1200px] w-full mx-auto">
      <div>
        <h1 className="text-[32px] font-semibold mb-[30px]">
          {detailBlog?.title && parse(detailBlog?.title)}
        </h1>
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <span className="text-[14px] opacity-30">
                {convertTimeToAgo(detailBlog?.createdAt)}
              </span>
            </div>
          </div>
          <div>Lưak chọn hêhhe</div>
        </div>
        <div>{detailBlog?.content && parse(detailBlog?.content)}</div>
        <div className="flex items-center gap-3 my-[20px]">
          {detailBlog?.tags?.map((item: any, index: number) => (
            <span
              key={index}
              className="px-[10px] py-[4px] bg-slate-200 text-[14px] text-slate-500"
            >
              {item}
            </span>
          ))}
        ...
        <div>
          <h1 className="mb-[10px] text-[20px] font-semibold">
            Post with same author
          </h1>
          <div>
            {listBlog?.listData
              ?.filter(
                (item: any, index: number) =>
                  item?.user?._id === detailBlog?.user?._id &&
                  item?._id !== idblog
              )
              ?.map((itemBlog: any, indexBlog: number) => (
                <div key={indexBlog} className="flex items-center gap-3">
                  <div className="w-[5px] h-[5px] rounded-full bg-black"></div>
                  <Link
                    to={`/blog/${itemBlog?._id}`}
                    className="hover:underline"
                  >
                    {itemBlog?.title && parse(itemBlog?.title)}
                  </Link>
                </div>
              ))}
          </div>
