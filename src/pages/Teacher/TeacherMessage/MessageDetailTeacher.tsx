...
const MessageDetailTeacher = ({
  ...
  const handleImageChange = (event: any) => {
    console.log(
      "🚀 ~ file: MessageDetailStudent.tsx:10 ~ handleImageChange ~ event:",
  ...
  const deleteImage = (id: number) => {
    ...
    const newArray = images.filter(
      (item: any, indexItem: number) => id !== indexItem
    );
    console.log(
      "🚀 ~ file: MessageDetailStudent.tsx:6 ~ MessageDetailStudent ~ images:",
      images
    );
    setImages(newArray);
  };
  ...
  return (
    <div className="h-full">
      ...
      <form
        className="p-[24px] border-[1px] border-[#E9EAF0]"
      ...
      >
          ...
          <div className="flex mb-[5px] gap-x-3">
            {images.map((image: any, index: number) => (
              <div className="relative" key={index}>
                ...
                />
                <IoIosCloseCircle
                  className="absolute top-1 right-2 text-[18px] cursor-pointer"
        ...
        <div className="flex gap-x-[20px]">
          ...
          <button
            type="submit"
            className="px-[18px] py-[12px] flex gap-x-[12px] items-center bg-[#FF6636] text-white"
          >
            <span className="font-semibold">Gửi Messages</span>
            <BsSend />
          </button>
        </div>
      </form>
